...
function ResponsiveStepper() {
  ...
  const stepperContainer = useRef<HTMLDivElement>(null);

  useLayoutEffect(() => {
    const updateStepWidth = () => {
      if (stepperContainer.current) {
        setStepWidth(stepperContainer.current?.clientWidth / steps.length);
      }
    };
    updateStepWidth();
  ...
  return (
    <div>
      ...
      <div
        ...
        className="w-[90%] mx-auto mt-10 relative isolate"
      >
        <div className="flex items-center justify-between ">
          {steps.map((step) => (
            <div
              className="grow flex flex-col items-center justify-center gap-2 z-10 relative"
              key={step.id}
            >
              <div
        ...
        {steps.length >= 1 && (
          ...
          >
            <div
              ...
              style={{
                width: (currentStep - 1) * stepWidth + "px",
              }}
            ></div>
          </div>
        )}
